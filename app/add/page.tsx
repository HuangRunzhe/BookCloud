...
export default function AddBook() {
  ...
  const [generatingPublishedDate, setGeneratingPublishedDate] = useState(false)
  ...
  const generatePublishedDate = async () => {
    if (!formData.title.trim() || !formData.author.trim()) {
      alert('请先填写书名和作者')
      return
    }
    ...
    try {
      setGeneratingPublishedDate(true)
      ...
      if (response.ok) {
        ...
        alert('AI生成出版日期失败，请重试')
      }
    } catch (error) {
      console.error('生成出版日期失败:', error)
      alert('生成出版日期失败，请重试')
    } finally {
      setGeneratingPublishedDate(false)
    }
  }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-center space-x-4 mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900">添加新图书</h1>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      type="button"
                      onClick={generateAuthor}
                      disabled={generatingAuthor || !formData.title.trim()}
                    ...
                    >
                      ...
                      <span>{generatingAuthor ? '生成中...' : 'AI生成'}</span>
                    </button>
                  </div>
                  <input
                    type="text"
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      type="button"
                      onClick={generatePublishedDate}
                      disabled={generatingPublishedDate || !formData.title.trim() || !formData.author.trim()}
                    ...
                    >
                      <Sparkles className="h-3 w-3" />
                      <span>{generatingPublishedDate ? '生成中...' : 'AI生成'}</span>
                    </button>
                  </div>
                  <input
                    type="date"
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      disabled={generatingCategory || !formData.title.trim() || !formData.description?.trim()}
                      className="flex items-center space-x-1 px-2 py-1 bg-orange-600 text-white text-xs rounded hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                      <Sparkles className="h-3 w-3" />
                      <span>{generatingCategory ? '生成中...' : 'AI生成'}</span>
                    </button>
                  </div>
                  <select
                    name="category"
